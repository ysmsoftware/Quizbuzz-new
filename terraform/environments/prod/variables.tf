...
variable "ops_vps_ip" {
  type        = string
  description = <<-EOT
    Ops dashboard VPS IP address, in CIDR form (e.g. "94.249.213.146/32").
    Allowed direct access to RDS Postgres on port 5432 — the ops dashboard
    runs on a separate VPS (not in this VPC) and needs to query both
    databases directly.

    THIS MUST STAY HERE, NOT IN THE AWS CONSOLE: the RDS security group is
    fully declared in terraform/modules/networking/main.tf, so any rule
    added by hand in the console gets silently deleted on the next
    `terraform apply` (including a routine go-live.sh/go-idle.sh mode
    switch). If the VPS's IP ever changes, update it here and re-apply —
    never add it directly in the AWS Console.
  EOT
}
