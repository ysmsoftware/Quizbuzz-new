...
# Ops dashboard VPS — third-party host (not in this VPC) that needs direct
# Postgres access to run cross-DB reads against the main app's database.
#
# WHY THIS IS A VARIABLE INSTEAD OF A CONSOLE-ADDED RULE:
# This IP used to be added by hand in the AWS Console directly on
# quizbuzz-rds-sg. Because that security group is fully, declaratively
# managed by the aws_security_group.rds resource below (inline ingress
# blocks = the COMPLETE authoritative rule set), every subsequent
# `terraform apply` — including a routine go-live.sh/go-idle.sh mode
# switch — reconciled the real security group back to exactly what was
# declared in code and silently deleted the console-added rule, breaking
# the Ops dashboard's DB connectivity until someone noticed and re-added it
# by hand again. Declaring it here means it's part of the permanent state
# and survives every future apply.
variable "ops_vps_ip" {
  description = "Ops dashboard VPS IP, allowed to reach RDS Postgres directly. Format: 1.2.3.4/32."
  type        = string
}
...
resource "aws_security_group" "rds" {
  ...
  # top-level `description` as immutable — ANY change to it forces Terraform
  # to destroy and recreate the entire security group (all rules included),
  # not just update the description in place. Keeping the original text here
  # avoids an unwanted -/+ replace on a resource that's otherwise a no-op.
  ...
  # Ops dashboard VPS — added so cross-DB access from the ops backend is
  # part of declared state instead of a console click that the next
  # `terraform apply` silently reverts. Description text matches exactly
  # what's already live in AWS for this rule (added by hand previously),
  # so this ingress block plans as a clean no-op rather than a churn/replace.
  ingress {
    from_port   = 5432
    to_port     = 5432
    protocol    = "tcp"
    cidr_blocks = [var.ops_vps_ip]
    description = "PostgreSQL from Ops VPS"
  }
