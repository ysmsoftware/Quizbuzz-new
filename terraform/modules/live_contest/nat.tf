...
resource "aws_eip" "nat" {
  ...
  lifecycle {
    # create_before_destroy ensures a new EIP is allocated before the old
    # one is released during a replacement, avoiding a brief period with
    # no NAT Gateway EIP. Does NOT fix the orphan problem on destroy —
    # that is handled in go-idle.sh by explicitly destroying the NAT GW
    # first (terraform destroy -target) before the full idle apply.
