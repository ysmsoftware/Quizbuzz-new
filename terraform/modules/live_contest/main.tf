...
# Rule 1b: Admin auth routes → admin-tg (MUST come before the /api/* rule)
# Admin dashboard auth cookies are set with COOKIE_DOMAIN=.ysmquizbuzz.com on
# the admin instance. If these routes hit quiz ASG instances they use a stale
# COOKIE_DOMAIN and auth breaks. Priority 12 = after websocket (10), before api (15).
resource "aws_lb_listener_rule" "admin_auth" {
  listener_arn = aws_lb_listener.https.arn
  priority     = 12

  condition {
    path_pattern { values = ["/api/v1/auth/admin/*", "/api/v1/org*"] }
  }
