...
# instead of the real ops-configured plan catalog. Same SSM param and
# fallback as the admin instance — see admin_instance/userdata.sh.tpl.
...
DB_POOL_MIN=2
# RDS max_connections ~85 for db.t3.micro. With up to 5 instances:
# floor(85 / 5) = 17. Set to 15 for safety margin.
# TODO: Add RDS Proxy to allow larger per-instance pools at scale.
DB_POOL_MAX=15
...
# ─────────────────────────────────────────────────────────────────────────────
# 7. SET FILE DESCRIPTOR LIMITS
#
# Each WebSocket connection consumes 1 file descriptor in Node.js.
# Linux default ulimit is 1024 — hit this and connections silently fail.
# For 1000 concurrent WS connections we need at least 2048 (connections
# + overhead for DB pool, Redis, log files, etc).
# Set to 65536 which is the standard production value — plenty of headroom.
# Must be done BEFORE docker starts so the daemon inherits the limit.
# ─────────────────────────────────────────────────────────────────────────────
...
# Apply immediately to the current shell (userdata runs as root)
ulimit -n 65536

# Make Docker daemon inherit the higher limit
mkdir -p /etc/systemd/system/docker.service.d
cat > /etc/systemd/system/docker.service.d/limits.conf << 'DOCKERLIMITS'
[Service]
LimitNOFILE=65536
DOCKERLIMITS
...
# ─────────────────────────────────────────────────────────────────────────────
# 8. LOGIN TO GHCR, PULL IMAGES, START CONTAINERS
# ─────────────────────────────────────────────────────────────────────────────
