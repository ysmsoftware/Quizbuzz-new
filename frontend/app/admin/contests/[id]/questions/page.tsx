...
function adjustPercentages(
    ...
    if (n <= 1) {
        return [targetSum]; // If only one item, it must take 100%
    }

    // Clamp new value
    newValue = Math.min(targetSum, Math.max(0, newValue));
...
function QuestionBankModal({
    ...
    const { autoGenerateMutation } = useQuestions();
