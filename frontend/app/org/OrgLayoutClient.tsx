...
export function OrgLayoutClient({ children }: { children: React.ReactNode }) {
    ...
    const onboardingQuery = useOnboardingStatus(readyForOnboarding);
        ...
        <div className="flex h-screen overflow-hidden bg-background">
            ...
            <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
                ...
                <header className="sticky top-0 z-40 border-b border-border/40 bg-background/80 backdrop-blur-xl shrink-0">
                    <div className="flex items-center justify-between px-2.5 sm:px-3 lg:px-6 py-2">
                        <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => setSidebarOpen(!sidebarOpen)}
                ...
                <main className="flex-1 overflow-y-auto">
                    <div className="p-2 sm:p-3 lg:p-4">
                        <WidgetErrorBoundary name="Org Content">
                            {children} </WidgetErrorBoundary>
                    </div>
                </main>
            </div>
