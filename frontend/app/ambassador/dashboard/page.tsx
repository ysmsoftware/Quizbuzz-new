...
/** Split into its own component so its hooks (campaign lists) are only ever called once an
 *  ambassador is confirmed logged in — calling them conditionally in the parent above would
    ...
    <div className="min-h-screen bg-background px-4 py-6 sm:py-8">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <section className="space-y-3">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">My Campaigns</h2>
          {joinedLoading ? (
              ...
              {availableCampaigns.map((campaign) => (
                <Card key={campaign.id} className="border-border/50">
                  <CardContent className="py-4 flex items-center justify-between gap-3">
                    ...
                    <Button size="sm" disabled={applyLoading} onClick={() => handleApply(campaign.id)}>
                      Apply
                    </Button>
                  </CardContent>
                </Card>
