...
export default function AmbassadorLoginPage() {
  ...
  const handleRequestOtp = async (data: EmailFormData) => {
    setApiError('');
    try {
      await requestOtp({ email: data.email, organizationId: orgSlug });
      setEmail(data.email);
      setStep('otp');
    } catch (err: any) {
  ...
  const handleVerifyOtp = async () => {
    ...
    setOtpError('');
    try {
      await verifyOtp({ email, organizationId: orgSlug, otp });
      router.push(`/ambassador/${orgSlug}/dashboard`);
    } catch (err: any) {
              ...
              <CardContent className="space-y-6">
                ...
                <p className="text-xs text-center text-muted-foreground">
                  ...
                  <button
                    ...
                    onClick={() => requestOtp({ email, organizationId: orgSlug })}
                  >
                    Resend
                  </button>
                </p>
