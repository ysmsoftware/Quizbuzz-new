...
export async function apiClient<T = unknown>(
    ...
    try {
        data = (await res.json()) as ApiResponse<T> | ApiError;
    } catch (parseErr) {
        if (res.status === 413) {
            throw new ApiRequestError(
                "PAYLOAD_TOO_LARGE",
                "File size is too large to upload. Please choose a smaller image (max 2 MB).",
                undefined,
                undefined,
                413
            );
        ...
        throw new ApiRequestError(
            "INVALID_RESPONSE",
            `Server returned an invalid response (${res.status} ${res.statusText || 'Error'}).`,
            undefined,
            undefined,
            res.status
        );
